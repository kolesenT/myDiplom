...
<div class="container">
    <nav class="navbar navbar-expand-lg bg-body-tertiary border-bottom border-primary mb-4">
        <div class="container-fluid">
            <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-mortarboard"
                 viewBox="0 0 16 16">
                <path
                                ...
                                <ul class="dropdown-menu">
                                    @foreach(\App\Models\SchoolClass::query()->orderBy('num')->get() as $item)
                                        <li><a class="dropdown-item"
                                               href="{{route('schedule.list', ['schoolClass'=>$item->id])}}">
                        ...
                        @can('view', \App\Models\Journal::class)
                            <li class="nav-item dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    @foreach(\App\Models\SchoolClass::query()->orderBy('num')->get() as $item)
                                        <li><a class="dropdown-item"
                                               href="{{route('journal.show', ['schoolClass'=>$item->id])}}">
                                                {{$item->num}} {{$item->letter}} класс</a></li>
                                    @endforeach
                                </ul>
                            </li>
                ...
                @if(auth()->check())
                    <div class="d-flex">
                        <form action="{{route('logout')}}" method="post" class="form-check-inline">
                            @csrf
                            <p class="d-inline-block">Вы вошли
                                как {{\Illuminate\Support\Facades\Auth::user()->name}}</p>
